...
print_message "$YELLOW" "Web UI & API Endpoints (through Kong Gateway):"
if [ "$SERVICE_TYPE" == "LoadBalancer" ]; then
    echo "  - Web UI (Angular): http://${EXTERNAL_ENDPOINT}:${PROXY_HTTP_PORT}/"
    ...
    echo "  - Web UI (Angular): http://localhost:${PROXY_HTTP_PORT}/"
